...
// Helper to get OXYY API Key
const getOxyyKey = (providedKeys?: string[]) => {
  if (providedKeys && Array.isArray(providedKeys) && providedKeys.length > 0) {
    const validKeys = providedKeys.filter(k => k && !k.includes('YOUR_API_KEY') && k !== 'MY_OXYY_API_KEY');
    if (validKeys.length > 0) {
      return validKeys[Math.floor(Math.random() * validKeys.length)];
    }
  }

  const apiKey = process.env.OXYY_API_KEY;
  if (!apiKey) {
    throw new Error('OXYY_API_KEY is not defined in environment variables and no valid keys were provided in the request.');
  }
  if (apiKey === 'MY_OXYY_API_KEY' || apiKey.includes('YOUR_API_KEY')) {
    throw new Error('Invalid OXYY_API_KEY detected. Please configure your API key.');
  }
  return apiKey;
};
...
const AUTO_CONFIG = {
  ...
  oxyyApiKeys: process.env.OXYY_API_KEYS ? process.env.OXYY_API_KEYS.split(',') : [],
  model: process.env.OXYY_MODEL || 'gemini-2.5-pro',
...
const processFile = async (fileId: string, fileUrl: string, config: any) => {
  ...
  try {
    ...
    // 2. Generate content with OXYY
    updateJob(jobId, { progress: 30, message: 'Generating content with OXYY API...' });
    const apiKey = getOxyyKey(config.oxyyApiKeys);
    const selectedModel = config.model || 'gemini-2.5-pro';
    ...
    try {
      ...
      if (!generatedContent) throw new Error('OXYY returned empty content');
    } catch (apiError: any) {
      throw new Error(`OXYY generation failed: ${apiError.message}`);
    }
...
const runAutomation = async () => {
  ...
  try {
    ...
    const filesToProcess = pendingFiles.filter((file: any) => {
      return !Array.from(jobs.values()).some(j => j.fileId === file.id && (j.status === 'processing' || j.status === 'pending'));
    });

    if (filesToProcess.length === 0) return;

    let index = 0;
    const CONCURRENCY_LIMIT = 50; // High concurrency for scaling
...
app.post('/api/generate', async (req, res) => {
  const { fileId, fileUrl, cloudinaryConfig, oxyyApiKeys, model } = req.body;
